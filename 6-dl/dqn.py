...
def eps_by_episode(epoch): return epsilon_final + \
    (epsilon_start-epsilon_final)*math.exp(-1.0*epoch/epsilon_decay)
...
class DQN(nn.Module):
    ...
    def __init__(self, num_inputs, num_actions):
        ...
        self.layers = nn.Sequential(
            nn.Linear(self.num_inputs, num_neurons),
            nn.ReLU(),
            nn.Linear(num_neurons, num_neurons),
            nn.ReLU(),
            nn.Linear(num_neurons, self.num_actions)
        )
...
def compute_td_loss(batch_size):
    ...
    next_state = autograd.Variable(torch.FloatTensor(
...
for episode in tr:
    tr.set_description(
        f'Agent training (episode {episode+1}) - AVG reward {tot_reward/(episode+1)})')
    tr.refresh()
