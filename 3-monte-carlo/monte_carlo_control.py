...
def monte_carlo_e_soft(env, episodes=100, policy=None, epsilon=.1, test_policy_freq=1000):
    if not policy:
    ...
    for e in range(episodes):
        ...
        for i in reversed(range(0, len(episode))):
            ...
            if not state_action in [(x[0], x[1]) for x in episode[0:i]]:
                ...
                Q[s_t][a_t] = sum(returns[state_action]) / \
...
policy = monte_carlo_e_soft(env, episodes=50000, epsilon=.01)
