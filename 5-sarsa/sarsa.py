...
def expect_epsilon(t):
    return min(0.015, 1.0-math.log10((t+1)/220.0))
...
def expect_alpha(t):
    return min(0.1, 1.0-math.log10((t+1)/125.0))
...
def update_SARSA(observation, reward, action, ini_obs, next_action, t):
    ...
    Q_table[tuple(ini_interval)][action] += max(0.4, expect_alpha(t)) * \
...
for episode in range(episodes):
    ...
    while (done == False):
        ...
        interval = to_discrete_states(observation)
        next_action = get_action(next_obs, t)
        update_SARSA(next_obs, reward, action, observation,
                     next_action, episode)
